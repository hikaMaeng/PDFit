...
export const tagsApi = {
  ...
  delete: async (tag: string) => { await deleteCachedTag(tag); return request<{ ok: boolean }>(`/tags/${encodeURIComponent(tag)}`, { method: 'DELETE' }); },
  updateColor: async (tag: string, color: string) => { await updateCachedTagColor(tag, color); return request<{ ok: boolean }>(`/tags/${encodeURIComponent(tag)}/color`, {
  ...
  /** PDF에 태그 추가 */
  addTag: async (folder: string, filename: string, tag: string) => { await mutateCachedTag(folder, filename, tag, false); return request<{ ok: boolean }>(
      `/tags/book/${encodeURIComponent(folder)}/${encodeURIComponent(filename)}`,
      {
        method: 'POST',
  ...
  /** PDF에서 태그 제거 */
  removeTag: async (folder: string, filename: string, tag: string) => { await mutateCachedTag(folder, filename, tag, true); return request<{ ok: boolean }>(
